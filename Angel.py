...
class Angel(object):
    ...
    def execute(self):
        ...
        for ego in xrange(0, self.total_nodes):
            ...
            # progress bar update
            percentage = float(actual * 100) / self.total_nodes
            actual += 1
            if percentage > old_p:
                hashes = '#' * int(percentage / 10)
                spaces = ' ' * (bar_length - len(hashes))
                sys.stdout.write(
                    "\rExec:  [{0}] {1}% {2}/{3}".format(hashes + spaces,
                                                         int(percentage), actual, self.total_nodes))
                sys.stdout.flush()
                old_p = percentage
    ...
    def __clean_communities(self, it):
        ...
        for c in com_sorted:
            ...
            # progress bar update
            percentage = float(actual * 100) / len(com_sorted)
            if percentage > old_p:
                hashes = '#' * int(percentage / 10)
                spaces = ' ' * (bar_length - len(hashes))
                sys.stdout.write(
                    "\rClean {0}: [{1}] {2}% {3}/{4}".format(it, hashes + spaces,
                                                             int(percentage), actual, len(com_sorted)))
                sys.stdout.flush()
                old_p = percentage
            actual += 1
        ...
        for c in community_to_nodes:
            ...
            for node in actual_community:
                ...
                if node.index in self.node2com:
                    comsids.extend(self.node2com[node.index].keys())
